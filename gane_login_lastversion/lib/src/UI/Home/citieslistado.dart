...
class CitiesListados extends StatefulWidget{

  final from;

  CitiesListados({this.from});

  _stateCitiesListados createState()=> _stateCitiesListados();
}

class _stateCitiesListados extends State<CitiesListados> with  TickerProviderStateMixin, WidgetsBindingObserver{
  ...
  void initState(){
    ...
    WidgetsBinding.instance!.addPostFrameCallback((_){
      Future.delayed(const Duration(milliseconds: 450), () {
        singleton.notifierCitiesListtSearch.value = cityList(code: 1,message: "No hay nada", status: false, data: DatacityList(cities:<Cities>[] ) );
        launchFetch();
      });
              ...
              children: [
                ...
                Expanded(
                  child: Container(
                      ...
                      child: ValueListenableBuilder<cityList>(
                          valueListenable: singleton.notifierCitiesListtSearch,
                          builder: (context,value,_){

                            return ListView.builder(
                              padding: EdgeInsets.only(top: 0,left: 0,right: 0),
                              scrollDirection: Axis.vertical,
                              itemCount: value.code== 1 ? 10 : value.data!.cities!.length,
                              itemBuilder: (BuildContext context, int index){

                                return value.code==1 ? utils.PreloadRefarreal() :
                                InkWell(
                                  onTap: (){
                                    singleton.city = value.data!.cities![index]!.name!;
                                    print(singleton.city);
                                    Navigator.pop(context);
                                  },
                                  child: Container(
                                    //color:Colors.red,
                                            ...
                                            children: <Widget>[
                                              ...
                                              Expanded(
                                                child: Container(
                                                  ...
                                                  child: Text(value.data!.cities![index]!.name!,
                                                      textScaleFactor: 1.0,
                                                      style: TextStyle(
  ...
  void _onMessageChanged(String value) {
    ...
    if (value.trim().isEmpty) {
      ...
      singleton.notifierCitiesListtSearch.value = cityList(code: 1,message: "No hay nada", status: false, data: DatacityList(cities:<Cities>[] ) );
      singleton.notifierCitiesListtSearch.value = singleton.notifierCitiesList.value;
      return;

    }else{


      singleton.notifierCitiesListtSearch.value = cityList(code: 100,message: "No hay nada", status: false, data: DatacityList(cities:<Cities>[] ) );
      List<Cities>? data = [] ;

      for (int i = 0; i < singleton.notifierCitiesList.value.data!.cities!.length ; i++) {
        var country = singleton.notifierCitiesList.value.data!.cities![i]!.name!.toLowerCase();
        ...
        if (country.contains(myvalue)){
          data?.add(singleton.notifierCitiesList.value.data!.cities![i]);
          print("");
        }
      }

      singleton.notifierCitiesListtSearch.value.data!.cities = data;
      return;
    }
