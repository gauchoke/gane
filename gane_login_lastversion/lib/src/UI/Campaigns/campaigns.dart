...
class Campaign extends StatefulWidget{
  ...
  _stateCampaign createState()=> _stateCampaign();
}

class _stateCampaign extends State<Campaign> with  TickerProviderStateMixin, WidgetsBindingObserver{
  ...
  void _onLoading() async{
    ...
    if(singleton.notifierLookRoom.value.code == 100){
      ...
      if(singleton.notifierLookRoom.value.data!.page! >= (singleton.GaneRoomPages + 1)){
        singleton.GaneRoomPages = singleton.GaneRoomPages + 1;
        try {
          final result = await InternetAddress.lookup('google.com');
          if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
            print('connected');
            servicemanager.fetchMiraRoom(context,"agregar");
  ...
  Widget _Example01Tile(BuildContext context, ResultSL item, int index){
    ...
    return GestureDetector(
      onTap: () {
        ...
        double points = double.parse(item.ads![0]!.pointsAds!) + double.parse(item.pointsAdditional!);
        singleton.notifierPointsGaneRoom.value = points.toString();
        url = item.ads![0]!.url!;
  ...
  void showAlertWonPoints(){
    ...
    Future.delayed(const Duration(milliseconds: 0), () {
      ...
      /// agregado
      Navigator.push(context, PageTransition(type: PageTransitionType.rightToLeftWithFade, duration: Duration(milliseconds: 350), child: WinsPoints(points: singleton.notifierPointsGaneRoom.value, image: singleton.notifierLookRoom.value.data!.result![itemSelected].ads![0]!.adImages!, onNextAd: onAnimationCoin,back: true,) ));
