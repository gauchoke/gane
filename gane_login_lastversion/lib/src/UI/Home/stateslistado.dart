...
class StatesListados extends StatefulWidget{

  final from;

  StatesListados({this.from});

  _stateStatesListados createState()=> _stateStatesListados();
}

class _stateStatesListados extends State<StatesListados> with  TickerProviderStateMixin, WidgetsBindingObserver{
  ...
  void initState(){
    ...
    WidgetsBinding.instance!.addPostFrameCallback((_){
      Future.delayed(const Duration(milliseconds: 450), () {
        singleton.notifierStatesListSearch.value = statesList(code: 1,message: "No hay nada", status: false, );
        launchFetch();
      });
              ...
              children: [
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Container(
                          ...
                          child: Text(Strings.searchstates,
                            textAlign: TextAlign.left,
                            textScaleFactor: 1.0,
                            style: TextStyle(fontFamily: Strings.font_regular, color: CustomColors.greyplaceholder, fontSize: 15,),
                          ),
                ...
                Expanded(
                  child: Container(
                      ...
                      child: ValueListenableBuilder<statesList>(
                          valueListenable: singleton.notifierStatesListSearch,
                          builder: (context,value,_){

                            return ListView.builder(
                              padding: EdgeInsets.only(top: 0,left: 0,right: 0),
                              scrollDirection: Axis.vertical,
                              itemCount: value.code== 1 ? 10 : value.data!.states!.length,
                              itemBuilder: (BuildContext context, int index){

                                return value.code==1 ? utils.PreloadRefarreal() :
                                InkWell(
                                  onTap: (){
                                    singleton.state = value.data!.states![index]!.name!;
                                    singleton.city = "";
                                            ...
                                            children: <Widget>[
                                              ...
                                              Expanded(
                                                child: Container(
                                                  ...
                                                  child: Text(value.data!.states![index]!.name!,
                                                      textScaleFactor: 1.0,
                                                      style: TextStyle(
  ...
  void _onMessageChanged(String value) {
    ...
    if (value.trim().isEmpty) {
      ...
      singleton.notifierStatesListSearch.value = statesList(code: 1,message: "No hay nada", status: false,  data: DatastatesList(states:<States>[] ) );
      singleton.notifierStatesListSearch.value = singleton.notifierStatesList.value;
      return;

    }else{


      singleton.notifierStatesListSearch.value =  statesList(code: 100,message: "No hay nada", status: false, data: DatastatesList(states:<States>[] )  );
      List<States>? data = [] ;

      for (int i = 0; i < singleton.notifierStatesList.value.data!.states!.length ; i++) {
        var country = singleton.notifierStatesList.value.data!.states![i]!.name!.toLowerCase();
        ...
        if (country.contains(myvalue)){
          data?.add(singleton.notifierStatesList.value.data!.states![i]);
          print("");
        }
      }

      singleton.notifierStatesListSearch.value.data!.states = data;
      return;
    }
